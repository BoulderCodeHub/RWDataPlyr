...
std::vector< std::vector<std::string> > parse_rdf_old_meta_old(std::vector<std::string> rdf) {
...
std::vector<std::string> parse_line_old(std::string line) {
  size_t pos;
  std::string token;
...
std::vector< std::vector<std::string> > build_table_old(std::vector<std::string> vals, std::vector<std::string> timesteps) {
...
std::vector<std::string> get_year_month_old(std::string timestep) {
...
int get_n_runs_old(std::vector< std::vector<std::string> > meta) {
...
std::vector< std::vector<std::string> > parse_rdf_old(std::vector<std::string> rdf, int num_traces) {
  ...
  for (trace = 1; trace <= num_traces; trace++) {
    ...
    for (size_t tt = 0; tt < num_time_steps; tt++) {
      ...
      ym = get_year_month_old(rdf.at(i));
    ...
    while (rdf.at(i) != "END_RUN") {
      ...
      i += 1;
      atts.push_back(parse_line_old(rdf.at(i))); // should be scale
      i += 1;
      ...
      tmp_table = build_table_old(vals, timesteps);
...
DataFrame rdf_to_rwtbl_cpp_old(std::vector<std::string> rdf) {
  ...
  meta = parse_rdf_old_meta_old(rdf);
  num_runs = get_n_runs_old(meta);
  rwtbl = parse_rdf_old(rdf, num_runs);
  ...
  //StringVector val2(rwtbl.at(0).size());
  //val2 = rwtbl.at(0);
