...
RiverWare is a generalized river system modeling tool. 
...
1. Reading and manipulating a single scenario
    - `read_rdf()` 
    - `read_rw_csv()`
1. Summarizing multiple slots of data from a singel scenario
    - `rwd_agg`
    - `rdf_aggregate()`
1. Aggregating and summarizing many scenarios
    - `rw_scen_aggregate()`
    - legacy workflow (`getDataForAllScens()`)

## Reading RiverWare Data

- `read_rw_csv(x)` ~= `rdf_to_rwtbl(read_rdf(y))` depending on options of the csv output selected in RiverWare

## Summarizing Scenario Data
...
The `read_rdf()` function will read rdf data into R. The function returns a multi-level list that contains all meta data embedded in an rdf file, including the start and end timesteps, the number of traces, units of each slot, and RiverWare object type for each slot.  
...
To allow the user to see the slots that are included in the rdf file once it is read into R, the function `rdf_slot_names()` will return all of the slots included in the rdf file.
