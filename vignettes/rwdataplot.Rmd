...
## Background
<span style = "color:red">*Need edits to this section*</span>
...
Then, `getMinAnnValue` can find and return the annual minimum values.
...
### Aggregate Multiple Scenarios to a Single Data Frame

**May want seperate section for the table so that each column can be explained.**

First, the slot aggregation list is created from the slot aggregation table. The slot aggregation table can either be created in R, or read in from an external csv. In either case, the table should take on the following form:
