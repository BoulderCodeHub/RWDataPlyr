...
`read_rdf()` returns an `rdf` object; to be useful, the rdf object must be converted to a `tbl_df` using `rdf_to_rwtbl()` or individual slot data can be obtained using `rdf_get_slot()`. `rdf_get_slot()` creates a timestep by traces matrix for a single slot. Several useful summary functions are available to summarize a single slot: `rwslot_annual_sum()`, `rwslot_annual_min()`, `rwslot_annual_max()`. A typical workflow to obtain the minimum annual Mead pool elevation from the included sample data might be:
...
`rwd_agg` objects are data.frames with specific column names and expected values in those columns. The expected column names are `file`, `slot`, `period`, `summary`, `eval`, `t_s`, and `variable` in that order. It is up to the user to specify which rdf file contains each slot. In a general case, the user specifies the `slot` that is found in a specific rdf file (`file`). A `summary` function is applied to a subset `period` of the `slot` data, and then compared (`eval`) to a threshold (`t_s`) and saved as the `variable`. In order to specify how each slot should be aggregated, each column should
 include specific keywords, which are described below. 

* *file:* specifies the rdf file that contains the slot.
* *slot:* the full RiverWare slot name, i.e., "Obect.Slot".
...
Often times, it will be easier to define the table in Excel and then read it in as a csv using `rwd_agg(read.csv(x, stringsAsFactors = FALSE))` but below shows an example of defining it directly in R. 
...
After the `rwd_agg` is specified the object is passed to `rdf_aggregate()` along with a few parameters specify where the data are stored, and a tbl_df is returned:
...
To compare results from multiple studies, i.e., the different top level CRSS.JanNNNN folders in figure \@ref(fig:dirStruct), `scen_dir` should point to "C:/user/crss" and `scenarios` should be assigned to `c("CRSS.Jan2017/ISM1988_2014,2007Dems,IG,Most", "CRSS.Jan2018/ISM1988_2014,2007Dems,IG,Most")`. 
