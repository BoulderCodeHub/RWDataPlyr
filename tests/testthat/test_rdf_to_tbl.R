...
rdftbl2 <- rw_rdf_to_tbl(keyRdf, add_ym = FALSE)
...
test_that("invalid add_ym values cause errors", {
  expect_error(rw_rdf_to_tbl(keyRdf, add_ym = NA))
  expect_error(rw_rdf_to_tbl(keyRdf, add_ym = "true"))
  expect_error(rw_rdf_to_tbl(keyRdf, add_ym = 7))
  expect_error(rw_rdf_to_tbl(keyRdf, add_ym = c(FALSE, TRUE, NA)))
})

# check the scenario option -------------
rdftbl2 <- rw_rdf_to_tbl(keyRdf, scenario = "DNF,CT")
rdftbl3 <- rw_rdf_to_tbl(keyRdf, scenario = 1, add_ym = FALSE)
test_that("dimensions are as expected with no ym", {
  expect_true(nrow(rdftbl2) > 0)
...
test_that("scenario options error properly", {
  expect_error(rw_rdf_to_tbl(keyRdf, scenario = c("DNF,CT", "DNF,C1")))
  expect_error(rw_rdf_to_tbl(keyRdf, scenario = 1:6, add_ym = FALSE))
  expect_error(rw_rdf_to_tbl(keyRdf, scenario = character(0), add_ym = FALSE))
})

# check the keep_cols option ------------
rdftbl2 <- rw_rdf_to_tbl(keyRdf, keep_cols = TRUE)
rdftbl3 <- rw_rdf_to_tbl(keyRdf, keep_cols = c("ObjectName", "Unit"), scenario = 1)
...
test_that("keep_cols warnings post correctly", {
  expect_warning(
    tmp <- rw_rdf_to_tbl(keyRdf, keep_cols = c("ObjectName", "missing")),
    paste0(
  ...
  expect_warning(
    tmp <- rw_rdf_to_tbl(keyRdf, keep_cols = c("ObjectName", "missing", "a")),
    paste0(
      "The following columns specified by 'keep_cols' were not found in the rwtbl:\n",
      "    missing, a"
    )
...
context("check rw_rdf_to_tbl with annual rdf file")
...
context("check rw_rdf_to_tbl() internal helpers")
