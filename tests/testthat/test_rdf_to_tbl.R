...
rdftbl2 <- rdf_to_rwtbl(keyRdf, add_ym = FALSE)
...
test_that("invalid add_ym values cause errors", {
  expect_error(rdf_to_rwtbl(keyRdf, add_ym = NA))
  expect_error(rdf_to_rwtbl(keyRdf, add_ym = "true"))
  expect_error(rdf_to_rwtbl(keyRdf, add_ym = 7))
  expect_error(rdf_to_rwtbl(keyRdf, add_ym = c(FALSE, TRUE, NA)))
})

# check the scenario option -------------
rdftbl2 <- rdf_to_rwtbl(keyRdf, scenario = "DNF,CT")
rdftbl3 <- rdf_to_rwtbl(keyRdf, scenario = 1, add_ym = FALSE)
test_that("dimensions are as expected with no ym", {
  expect_true(nrow(rdftbl2) > 0)
