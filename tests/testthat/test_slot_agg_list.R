...
test_that("slot_agg_list returns proper errors", {
  expect_error(slot_agg_list(matrix(1:6, ncol = 3)), 
               "iData is a matrix with 3 columns. There should either be 4 or 5 columns.")
  expect_error(slot_agg_list(matrix(1:6, ncol = 6)), 
               "iData is a matrix with 6 columns. There should either be 4 or 5 columns.")
  expect_error(slot_agg_list(1:3), 
               "`iData` is not a matrix, nor can it be converted to an Nx4 or Nx5 matrix")
  expect_error(slot_agg_list(1:17),
               "`iData` is not a matrix, nor can it be converted to an Nx4 or Nx5 matrix")
  expect_warning(slot_agg_list(c('KeySlots.rdf','Mead.Pool Elevation','AnnMax',NA, 'KeySlots.rdf','Powell.Outflow','AnnMin',NA)),
                 "Attempting to convert iData to a N x 4 matrix. Results may be unexpected. Probably better to stop and pass a matrix to slot_agg_list.")
  expect_warning(slot_agg_list(c('KeySlots.rdf','Mead.Pool Elevation','EOCY',NA)),
                 "Attempting to convert iData to a N x 4 matrix. Results may be unexpected. Probably better to stop and pass a matrix to slot_agg_list.")
  expect_warning(slot_agg_list(c('KeySlots.rdf','Mead.Pool Elevation','EOCY',NA, "var1",
                                     'KeySlots.rdf','Mead.Pool Elevation','EOCY',NA, "var1",
                                     'KeySlots.rdf','Mead.Pool Elevation','EOCY',NA, "var1")),
                 "Attempting to convert iData to a N x 5 matrix. Results may be unexpected. Probably better to stop and pass a matrix to slot_agg_list.")
  expect_warning(slot_agg_list(c('KeySlots.rdf','Mead.Pool Elevation','EOCY',NA, "var1")),
                 "Attempting to convert iData to a N x 5 matrix. Results may be unexpected. Probably better to stop and pass a matrix to slot_agg_list.")
  expect_error(slot_agg_list(file.path("some", "crazy", "file.txt")),
               paste(file.path("some", "crazy", "file.txt"),'does not exist.'))
  expect_error(slot_agg_list(samFail),
  ...
  expect_error(
    slot_agg_list(matrix(c('KeySlots.rdf','Mead.Pool Elevation','Weird',NA,
  ...
  expect_error(
    slot_agg_list(matrix(c('KeySlots.rdf','Mead.Pool Elevation','EOCY',NA,
                               "KeySlots.rdf", "Mead.Pool Elevation", "TooWeird", NA),
                             nrow = 2, byrow = TRUE)),
    paste0("Invalid aggregation methods:\n    ", 
...
sal <- slot_agg_list(system.file("extdata", "SlotAggTable.csv", package = "RWDataPlyr"))
sam <- matrix(c(
